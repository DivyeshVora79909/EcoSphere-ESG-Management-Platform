...
import { Search, Plus, Target, Zap, Users } from "lucide-solid";
...
export default function ChallengeSystem() {
  ...
  createEffect(() => setPageMeta("Challenge Matrix", "challenges"));

  const [search, setSearch] = createSignal("");
  const [statusFilter, setStatusFilter] = createSignal("Active");

  // Query fetches challenges AND their O(1) completions from the vector view
  const challengesQuery = createQuery(() => ({
    queryKey: ["esg_challenges"],
    queryFn: async () => {
      const res = await fetchQuery(`
        SELECT 
          id, a_name, a_xp_reward, a_status, 
          a_category.a_name AS category_name,
          (SELECT VALUE total_qty FROM type::record('v_tx_to', [id, 1]))[0] AS completions
        FROM challenge 
  ...
  const filteredChallenges = createMemo(() => {
    let nodes = challengesQuery.data || [];
    if (statusFilter() !== "All")
    ...
    <div class="flex items-center justify-between gap-4 w-full">
      ...
      <div class="flex items-center gap-2">
        ...
        <button
          ...
          onClick={() =>
            openDrawer("Deploy Challenge", () => <ChallengeForm />)
          }
        >
          <Plus size={16} /> <Target size={16} /> New
        </button>
      </div>
    </div>
  );
        ...
        >
          <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-[var(--app-pad)]">
            {filteredChallenges().map((chal) => (
              <div
                class={`card bg-base-100 shadow-sm border hover:shadow-md transition-all cursor-pointer group ${chal.a_status !== "Active" ? "opacity-70 border-base-300" : "border-primary/30 hover:border-primary"}`}
                onClick={() =>
                  openDrawer("Edit Challenge", () => (
                    <ChallengeForm baseChallenge={chal} />
                  ))
                }
              >
                <div class="card-body p-4 flex flex-col gap-3">
                  <div class="flex items-center justify-between">
                    <span
                      class={`badge badge-sm ${chal.a_status === "Active" ? "badge-success" : "badge-ghost"}`}
                    >
                      {chal.a_status}
                    </span>
                    <span class="text-xs font-mono text-base-content/40">
                      {chal.category_name || "No Category"}
                    </span>
                  </div>
                  <div class="flex items-center gap-3">
                    <div class="w-10 h-10 rounded-box bg-info/10 flex items-center justify-center text-info shrink-0">
                      <Target size={20} />
                    </div>
