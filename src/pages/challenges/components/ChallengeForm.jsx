...
export default function ChallengeForm(props) {
  ...
  const isUpdate = !!props.baseChallenge;

  const [name, setName] = createSignal(
    isUpdate ? props.baseChallenge.a_name : "",
  );
  const [xpReward, setXpReward] = createSignal(
    isUpdate ? props.baseChallenge.a_xp_reward : 50,
  );
  const [status, setStatus] = createSignal(
    isUpdate ? props.baseChallenge.a_status : "Active",
  );
  const [category, setCategory] = createSignal(
    isUpdate ? props.baseChallenge.a_category?.id || "" : "",
  );
  const [loading, setLoading] = createSignal(false);

  // STRICT TAXONOMY: Only fetch categories meant for challenges
  const [categories] = createResource(async () => {
    return unwrap(
      await fetchQuery(
        `SELECT id, a_name FROM category WHERE a_target_table = 'challenge'`,
      ),
    );
  });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (isUpdate) {
        ...
        `,
          {
            id: props.baseChallenge.id,
            name: name(),
            xpReward: Number(xpReward()),
            status: status(),
          },
        );
        toast.success("Challenge node updated.");
      } else {
        await fetchQuery(
          `
      ...
      qc.invalidateQueries({ queryKey: ["esg_challenges"] });
      closeDrawer();
    } catch (err) {
  ...
  return (
    <form onSubmit={handleSubmit} class="flex flex-col gap-4 h-full pb-4">
      <div class="form-control">
        <label class="label">
          <span class="label-text font-bold">Challenge Name</span>
        </label>
        <input
          type="text"
